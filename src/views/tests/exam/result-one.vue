              ...
              <div class="col-md-5  card right">
                ...
                <div>
                  ...
                  <div class="item">
                    ...
                    <div class="main-item">
                     {{$moment(this.exam.begin_time).format('YYYY-MM-DD HH:mm:ss')}}
                    </div>
                  </div>
                  <div class="item">
                    <div class="name">
                    ...
                    <div class="main-item">
                      {{$moment(this.exam.end_time).format('YYYY-MM-DD HH:mm:ss')}}
                    </div>
                  </div>
                  <div class="item">
                    <div class="name">
                      ...
                      >
                        ...
                        <ul class="test_answers pb-1">
                          <li
                              ...
                              :key="a"
                              :class="item.is_selected&&item.is_true?'test_answers-title active':(item.is_selected&&!item.is_true)?'test_answers-title wrong':'test_answers-title'"
                          >
                            <div class="mr-1" v-show="item.is_selected&&item.is_true">
                              ✔
                            </div>
                            <div class="mr-1" v-show="item.is_selected&&!item.is_true">
                              ✖
                            </div>
                            <div class="answer-one" v-html="item.name">
...
export default {
  ...
  methods:{
    getExam(){
      axios.get(`https://api.fastlms.uz/api/result/${this.examId}/student/${this.studentId}/`).then((res)=> {
        this.exam = res.data
    ...
    getQuestion(){
      axios.get(`https://api.fastlms.uz/api/student-select-answers-get-profile/${this.examId}/for/${this.studentId}/`).then((res)=> {
